...
class StudentsTest < ApplicationSystemTestCase
  ...
  test 'student views their Mountain Time for the current week' do
    sign_in_as_student_and_visit_profile
  ...
  test 'student views their Mountain Time for the previous week' do
    sign_in_as_student_and_visit_profile
    click_link 'Previous week'
    within('#tuesday') { assert_selector 'h5', text: 'Fake Previous Tuesday Activity' }
  end

  test 'student views their Mountain Time for the next week' do
    sign_in_as_student_and_visit_profile
    click_link 'Next week'
    within('#tuesday') { assert_selector 'h5', text: 'Fake Next Tuesday Activity' }
  end

  test 'student sees current Mountain Time teacher and not the form' do
    sign_in_as_student_and_visit_profile
    assert_no_selector('student_teacher_id')
    assert_selector('h3#mountain_time_teacher', text: users(:student).teacher.to_s)
  end
  ...
  test 'student specifies a teacher' do
    ...
    click_button 'Ok'
    assert_text 'Mountain Time teacher set to Mr. Valid'
    assert_selector 'h2', text: users(:student).to_s
  end
